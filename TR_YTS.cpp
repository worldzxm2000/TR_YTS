...
{
	QString name = QString::fromLocal8Bit("一体式墒情温湿度监测站");
	return name;
}
		...
		if (buff[i] == 0xaa)
		{
			...
			for (int j = i ; j < buff.length(); j++)
			{
				if (buff[j] == 0xdd)
				{
					...
					QString fileName = QCoreApplication::applicationDirPath() + "\\TR_YTS\\" + QString::number(frame.SrcAddr) + "\\" + current_day;
					QDir dir(fileName);
					if (!dir.exists())
...
void Transform2GPRSPerVolume(QString data, QJsonObject &json)
{
	...
	for (int i = 0; i < LayerCount; i++)
	{
		QString ST_ST = "ST_ST";
		//地温
		f.float_byte.low_byte = data[i * 4 + 5 + LayerCount * 8].unicode();
		f.float_byte.mlow_byte = data[i * 4 + 1 + 5 + LayerCount * 8].unicode();
		f.float_byte.mhigh_byte = data[i * 4 + 2 + 5 + LayerCount * 8].unicode();
		f.float_byte.high_byte = data[i * 4 + 3 + 5 + LayerCount * 8].unicode();
		json.insert(ST_ST+QString::number(i+1), QJsonValue(f.value));
		if (i==0)
		{
			//体积含水量
